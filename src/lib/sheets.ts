import { mockProjects } from "./mockProjects";
import { placeholderFabric } from "./placeholderFabric";
import type { FabricSwatch, QuiltProject } from "./types";

/**
 * Data layer for quilt projects.
 *
 * Primary source: a public Google Sheet, read via the `gviz/tq` endpoint (no
 * API key or service account needed — just share the sheet as "Anyone with
 * the link can view"). Two tabs are read:
 *
 *   Projects (GOOGLE_SHEET_NAME, defaults to "Projects")
 *     - one row per project, as documented in rowToProject below.
 *
 *   Fabrics (GOOGLE_FABRICS_SHEET_NAME, defaults to "Fabrics") — optional
 *     - one row per fabric swatch, joined onto its project by project_slug.
 *     - columns: project_slug, fabric_name, designer, colorway, image
 *     - if this tab doesn't exist yet, projects simply show no swatches.
 *
 * Until GOOGLE_SHEET_ID is set, or if a fetch fails, this falls back to
 * local mock data so the site can be developed and previewed without any
 * external dependency.
 */

const SHEET_ID = process.env.GOOGLE_SHEET_ID;
const SHEET_NAME = process.env.GOOGLE_SHEET_NAME ?? "Projects";
const FABRICS_SHEET_NAME = process.env.GOOGLE_FABRICS_SHEET_NAME ?? "Fabrics";

const EXPECTED_COLUMNS = [
  "id",
  "slug",
  "project_name",
  "status",
  "cover_image",
  "year",
  "date_started",
  "date_cut",
  "date_top_finished",
  "date_finished",
  "quilt_type",
  "dimensions",
  "pattern_name",
  "pattern_designer",
  "pattern_source",
  "pattern_link",
  "quilter_name",
  "quilting_design",
  "main_fabric",
  "fabric_brand",
  "fabric_designer",
  "fabric_collection",
  "batting",
  "backing",
  "binding",
  "techniques",
  "project_story",
  "notes",
  "lessons_learned",
  "photo_gallery",
  "featured",
] as const;

function splitList(value: string | undefined): string[] {
  if (!value) return [];
  return value
    .split(/[,|\n]/)
    .map((v) => v.trim())
    .filter(Boolean);
}

function rowToProject(row: Record<string, string>, index: number): QuiltProject {
  const gallery = splitList(row.photo_gallery);
  const fallbackCover = placeholderFabric(row.id || row.slug || String(index));

  return {
    id: row.id || String(index),
    slug: row.slug || `project-${index}`,
    projectName: row.project_name || "Untitled Project",
    status: row.status || "Finished",
    coverImage: row.cover_image?.trim() || fallbackCover,
    year: row.year || "",
    dateStarted: row.date_started || undefined,
    dateCut: row.date_cut || undefined,
    dateTopFinished: row.date_top_finished || undefined,
    dateFinished: row.date_finished || undefined,
    quiltType: row.quilt_type || undefined,
    dimensions: row.dimensions || undefined,
    patternName: row.pattern_name || undefined,
    patternDesigner: row.pattern_designer || undefined,
    patternSource: row.pattern_source || undefined,
    patternLink: row.pattern_link || undefined,
    quilterName: row.quilter_name || undefined,
    quiltingDesign: row.quilting_design || undefined,
    mainFabric: row.main_fabric || undefined,
    fabricBrand: row.fabric_brand || undefined,
    fabricDesigner: row.fabric_designer || undefined,
    fabricCollection: row.fabric_collection || undefined,
    fabrics: [],
    batting: row.batting || undefined,
    backing: row.backing || undefined,
    binding: row.binding || undefined,
    techniques: splitList(row.techniques),
    projectStory: row.project_story || undefined,
    notes: row.notes || undefined,
    lessonsLearned: row.lessons_learned || undefined,
    photoGallery: gallery.length > 0 ? gallery : [row.cover_image?.trim() || fallbackCover],
    featured: /^(true|yes|1)$/i.test(row.featured ?? ""),
  };
}

/** Parses the JSONP-wrapped response from Google's gviz/tq endpoint into rows keyed by header label. */
function parseGviz(text: string): Record<string, string>[] {
  const match = text.match(/google\.visualization\.Query\.setResponse\(([\s\S]*)\);?\s*$/);
  if (!match) throw new Error("Unrecognized gviz response");
  const json = JSON.parse(match[1]);
  const cols: string[] = json.table.cols.map((c: { label?: string; id?: string }, i: number) => {
    const label = (c.label || "").trim().toLowerCase().replace(/\s+/g, "_");
    return label || EXPECTED_COLUMNS[i] || `col_${i}`;
  });
  const rows: { c: ({ v: unknown; f?: string } | null)[] }[] = json.table.rows;

  return rows.map((r) => {
    const record: Record<string, string> = {};
    cols.forEach((col, i) => {
      const cell = r.c[i];
      // Prefer the formatted display string (`f`) over the raw value (`v`) —
      // Sheets returns dates as raw values like "Date(2023,2,1)", which only
      // `f` renders as the human-readable "March 2023".
      const value = cell?.f ?? cell?.v;
      record[col] = value === null || value === undefined ? "" : String(value);
    });
    return record;
  });
}

async function fetchSheetTab(sheetName: string): Promise<Record<string, string>[] | null> {
  if (!SHEET_ID) return null;

  const url = `https://docs.google.com/spreadsheets/d/${SHEET_ID}/gviz/tq?tqx=out:json&sheet=${encodeURIComponent(
    sheetName
  )}`;

  try {
    const res = await fetch(url, { next: { revalidate: 300 } });
    if (!res.ok) return null;
    const text = await res.text();
    return parseGviz(text);
  } catch {
    return null;
  }
}

async function fetchFromSheet(): Promise<QuiltProject[] | null> {
  const rows = await fetchSheetTab(SHEET_NAME);
  if (!rows || rows.length === 0) return null;
  return rows.map(rowToProject).filter((p) => p.slug && p.projectName);
}

/** Reads the optional Fabrics tab and groups swatches by project_slug. */
async function fetchFabricsBySlug(): Promise<Map<string, FabricSwatch[]>> {
  const map = new Map<string, FabricSwatch[]>();
  const rows = await fetchSheetTab(FABRICS_SHEET_NAME);
  if (!rows) return map;

  for (const row of rows) {
    const slug = row.project_slug?.trim();
    const fabricName = row.fabric_name?.trim();
    if (!slug || !fabricName) continue;

    const swatch: FabricSwatch = {
      fabricName,
      designer: row.designer?.trim() || undefined,
      colorway: row.colorway?.trim() || undefined,
      image: row.image?.trim() || undefined,
    };

    if (!map.has(slug)) map.set(slug, []);
    map.get(slug)!.push(swatch);
  }

  return map;
}

export async function getAllProjects(): Promise<QuiltProject[]> {
  const [fromSheet, fabricsBySlug] = await Promise.all([fetchFromSheet(), fetchFabricsBySlug()]);
  if (!fromSheet) return mockProjects;
  return fromSheet.map((p) => ({ ...p, fabrics: fabricsBySlug.get(p.slug) ?? [] }));
}

export async function getFeaturedProjects(): Promise<QuiltProject[]> {
  const projects = await getAllProjects();
  const featured = projects.filter((p) => p.featured);
  return featured.length > 0 ? featured : projects;
}
